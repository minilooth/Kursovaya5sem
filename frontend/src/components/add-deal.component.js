...
import { Card, Container, Form, Button, Col, Spinner } from 'react-bootstrap';
...
export default class AddDeal extends Component {
    ...
    onCarIdFocus = () => {
        this.setState({ isCarIdInvalid: false })
    }

    onUserIdFocus = () => {
        this.setState({ isUserIdInvalid: false })
    }

    onCarIdChange = (e) => {
        let car = this.state.cars.find(c => String(c.id) === e.target.value);

        this.setState({ 
            carId: e.target.value,
            image: e.target.value !== "" ? car.image : null
        })
    }
        ...
        if (!this.isFormInvalid) {
            DealService.add(this.state.userId, this.state.carId).then(
                response => {
                    ...
                    setTimeout(() => this.props.history.push("/admin/dealsList"), 5000);
                },
                error => {
    ...
    render() {
        ...
        if (this.state.isUsersLoading || this.state.isCarsLoading) {
            form = 
                    ...
                    <Form.Row>
                        <Form.Group as={Col} id="username">
                            ...
                            <Form.Control
                                ...
                                { this.state.users !== null && this.state.cars !== null ? (
                            ...
                            {this.state.isUserIdInvalid ? <span className="error">{this.state.userIdError}</span> : null}
                        </Form.Group>
                    </Form.Row>
                        ...
                        <div style={{width: "50%", display: "flex", alignItems: "center"}}>
                            <Form.Group id="photo" style={{paddingRight: "20px", width: "100%"}}>
                                ...
                                onChange={this.onCarIdChange}>
                                    <option value="" hidden>Выберите...</option>
                                    { this.state.cars !== null && this.state.users !== null ? (
                                ...
                                {this.state.isCarIdInvalid ? <span className="error">{this.state.carIdError}</span> : null}
                            </Form.Group>
                        </div>
