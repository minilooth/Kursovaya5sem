...
export default class EditDealModal extends Component {
    constructor(props) {
        super(props);
    ...
    getCars = (id) => {
        CarService.getNotSoldCarsAndCurrentDealCar(id).then(
            response => {
                this.setState({
                    isCarsLoading: false,
                    cars: response.data
    ...
    onEdit = () => {
        ...
        if (!this.isFormInvalid) {
            ...
            DealService.edit(this.state.id, this.state.userId, this.state.carId).then(
                response => {
    ...
    render() {          
        ...
        if (this.props.show && this.state.isDataGetted === false) {
            this.getCars(this.state.carId);
            this.getUsers();
        ...
        return(
            ...
            onHide={this.onHide}>
                <Modal.Header closeButton>
                    <Modal.Title><FontAwesomeIcon icon={faEdit}/>&nbsp;Редактирование сделки</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {formData}
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="primary" disabled={this.state.isDisabled} onClick={this.onEdit}><FontAwesomeIcon icon={faSave}/>&nbsp;Изменить</Button>
